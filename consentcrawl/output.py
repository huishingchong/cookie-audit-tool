...
PRE_EVIDENCE_COL = "pre_page_evidence"     # NEW
POST_EVIDENCE_COL = "post_page_evidence"   # NEW
...
def load_rows(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    # Always include mandatory basics
...
def build_report(rows: List[Dict[str, Any]], only_action: Optional[str], max_sites: Optional[int], include_value: bool) -> str:
    ...
    for r in rows:
        ...
        pre_ev = parse_evidence_list(r.get(PRE_EVIDENCE_COL))   # NEW
        post_ev = parse_evidence_list(r.get(POST_EVIDENCE_COL)) # NEW

        pre_seen_map = build_pages_seen(pre_ev)     # NEW
        post_seen_map = build_pages_seen(post_ev)   # NEW
        ...
        b.setdefault(group_key, []).append({
            ...
            "pre": pre,
            "post": post,
            "pre_seen": pre_seen_map,    # NEW
            "post_seen": post_seen_map,  # NEW
            "prefs": prefs,
            "toggles": toggles,
            "dbg": debug,
        })
